...
const MyBedBookings = () => {
    ...
    const payNow = (booking) => {
        navigate("/book-and-pay", { state: booking });
    };

    const getStatusBadge = (b) => {
        if (b.cancelled) return <span className="bg-red-50 text-red-600 px-2 py-1 rounded-full text-xs border border-red-200">Cancelled</span>;
        if (b.completed) return <span className="bg-green-50 text-green-600 px-2 py-1 rounded-full text-xs border border-green-200">Booked</span>;
        return <span className="bg-yellow-50 text-yellow-600 px-2 py-1 rounded-full text-xs border border-yellow-200">Pending Payment</span>;
    };
                                ...
                                <tr key={b._id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="border border-gray-200 px-4 py-3">
                                        {b.cancelled ? (
                                            ...
                                            <button
                                                className="px-3 py-1.5 text-xs bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition shadow-sm"
                                                onClick={() => payNow(b)}
                                            >
                                                Pay Now
                                            </button>
